...
namespace EL_DC_Charger.EL_DC_Charger.SerialPorts.smartro_tl3500bs.Packet.Child
{
    ...
    public class Smartro_TL3500BS_Packet_AddInfor_Deal_Request_Send_Request : Smartro_TL3500BS_Packet_Base_Send_Request
    {
        const int LENGTH_DATA = 339;
        public Smartro_TL3500BS_Packet_AddInfor_Deal_Request_Send_Request(EL_MyApplication_Base application, int channelIndex) :
            base(application, channelIndex, LENGTH_DATA, Smartro_TL3500BS_Constants.VALUE.JOB_CODE.G_ADDINFOR_APPROVAL_REQ)
        {
            mSendData_Data[0] = (byte)'1';

            int length = 30 + 20 + 40 + 20 + 50 + 100 + 50;
            int startIndex = 1 + 10 + 8 + 8 + 2;
            for(int i = 0; i < length; i++)
            {
                mSendData_Data[startIndex + i] = (byte)' ';
            }
        }
